const allowedConditions = ['New', 'Used', 'Homemade'];

const validateSoapInput = (req, res, next) => {
  ...
  if (!allowedConditions.includes(trimmedCondition)) {
    return res.status(400).json({ error: 'Condition must be New, Used, or Homemade' });
  }
...
module.exports = validateSoapInput;